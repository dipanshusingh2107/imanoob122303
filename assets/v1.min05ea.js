...
function buildEpslist(e, t) {
  ...
  for (epsavailable = Object.keys(e).length - n, o = 1; o <= epsavailable; o++)
    void 0 !== e[o - 1] && "-" != e[o - 1]
      ? (a += `<button class='playbutton btn btn-primary' onClick='openiframe(this,${o})'>${o}</button>`)
      : (a += `<button class='playbutton btn btn-primary hidden'>${o}</button>`),
      void 0 !== i[o] &&
  ...
  if (
    ...
    0 === e.eptotal &&
      1 === n &&
      (a =
        "<button class='loadmorebtn btn btn-primary' disabled>No ep available yet</button>"),
...
function openRequested() {
  ...
  if ("string" == typeof window.location.pathname.split("/")[3]) {
    var t,
      a = window.location.pathname.split("/")[3].substring(2);
  ...
  if (e) {
    ...
    else if ("function" == typeof gClass("playbuttonextra")[0].onclick)
      gClass("playbuttonextra")[0].click();
    else {
      var s = ("https://" + window.location.host + window.location.pathname)
        .split("/")
        .slice(0, 5)
        .join("/");
...
function loaded() {
  ...
  (-1 !== document.title.indexOf("(Uncensored)") || e) &&
    (isStorage("localStorage") &&
    "true" === localStorage.getItem("adultConfirmed")
      ? shownotif("This show is for mature audiences", 3e3)
    ...
    (a.style.display = "block"),
      a.addEventListener("click", function () {
        if (n) shownotif("Don't spam click");
          ...
          shownotif("Reloading...", 800), (gID("iframeplayer").src = e);
        }
      });
  }
  ...
  if ("object" == typeof fillerData)
    for (var l, s = gClass("playbutton"), r = 0; r < fillerData.length; r++)
      if ("string" == typeof fillerData[r] && -1 !== fillerData[r].indexOf("-"))
        for (
          var c = fillerData[r].split("-", 2),
            p = parseInt(c[1]) + 1,
            d = parseInt(c[0]);
          d < p;
          d++
        )
          (l = d - 1) < s.length && (s[l].style.backgroundColor = "#5e5e5e");
      else
        "number" == typeof fillerData[r] &&
          (l = fillerData[r] - 1) < s.length &&
          (s[l].style.backgroundColor = "#5e5e5e");
  "proxy" !== mode && enableSandboxBtn(),
    isChrome
      ? "proxy" !== mode
        ? ((gID("tips").innerHTML =
            'If you get Embed blocked error, change Chrome PDF setting <a href="https://cdn.animixplay.to/s/pdf.png" target="_blank">like this</a>.<br>This will fix the error. <i onClick="infoclose();" class="closebtn glyphicon glyphicon-remove-sign"></i>'),
  ...
  var u = 0;
  "boolean" == typeof requestNoAds && requestNoAds && (u = 1),
    $.ajax({
      url: ap2,
      type: "POST",
      data: { t: utoken, tmod: u },
      success: function (e) {
        var t = e.episode;
        if (null != t) {
          if (1 != t && 0 != t) {
          ...
          if (
            ...
            $("#premiumcrown").html(c),
              ...
              autolightoff && (gID("autolightsbtn").style.color = "#7fc3ff"),
              autoproxy && (gID("autoproxybtn").style.color = "#7fc3ff"),
              prefersub && (gID("preferbtn").style.color = "#7fc3ff"),
              accountautoplay &&
...
function buildSwitcher(e) {
  ...
  for (var i = 0; i < a; i++)
    e[i].includes("streaming.php")
      ? (t = "Vidstream")
      : e[i].includes("dm/embed.html") || e[i].includes("dailymotion")
      ? (t = "DM Multi")
  ...
  if (
    ...
    if ((a = atob(a)).includes(".m3u8"))
      ...
      if ("undefined" == typeof replacement) {
        ...
        (n.src = "https://anfruete.github.io/play/env.js"),
...
function togglePlaybackSave() {
  isPremium
    ? shownotif("Playback save is always on")
    : shownotif(notPremiumMsg);
}
var dismissNotif = !1,
  msgLimit = 3,
...
function handleMsg(e) {
  if (
    ...
    if ("string" == typeof e.data) {
      ...
      else if ("fallback" === s[0])
        "fullscreen" === s[1] ? fullscreenFallback(!0) : fullscreenFallback(!1);
      else if ("proxy" === s[0])
        if (
          (((reportedTele || "notexist" !== s[1]) && reportedTele) ||
            (reportedTele = !0),
          "force" === s[2])
        ) {
          var o = oldlink.split("?id=")[1].split("&")[0];
          gID("iframeplayer").src = "/goplyr.html#" + btoa(o);
          var i = gID("notifiaction");
          (i.style.animationName = "notifout"),
            setTimeout(function () {
              (i.style.display = "none"), (notifDisplayed = !1);
            }, 400);
        } else
          "notexist" === s[1] || "silent" === s[2]
            ? (shownotif(
...
function fullscreenFallback(e) {
  if (inFallbackFull !== e) {
    ...
    (t.src.includes(location.hostname) ||
      (/plyr.link|plyr.in/.test(t.src) && t.src.includes("player.html"))) &&
      ((o = !1), (a = t)),
      e
        ? ((inFallbackFull = !0),
          (a.style =
  ...
  metaPressed && "Meta" === e.key && (metaPressed = !1),
    metaPressed ||
      (("l" !== e.key && "L" !== e.key) ||
        ...
        lighttoggle(),
      ("]" === e.key || (e.shiftKey && "n" === e.key.toLowerCase())) &&
        playnext(),
      ("[" === e.key || (e.shiftKey && "p" === e.key.toLowerCase())) &&
        $(currentbutton).prev().click());
}
